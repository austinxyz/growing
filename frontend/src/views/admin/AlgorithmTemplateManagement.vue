        ...
        <div class="flex-1 overflow-y-auto px-6 py-4">
          ...
          <div v-else-if="availableQuestions.length === 0" class="text-center py-8">
            ...
            <p class="text-sm text-gray-500">
              {{ selectedFocusAreaId ? '该分类下暂无试题' : '请选择Focus Area或输入搜索词' }}
            </p>
          </div>
          <div v-else class="space-y-2">
            <div
...
const loadQuestionsByFocusArea = async () => {
  ...
  try {
    ...
    console.log('Loading questions for Focus Area:', selectedFocusAreaId.value)
    ...
    availableQuestions.value = filtered
  } catch (error) {
