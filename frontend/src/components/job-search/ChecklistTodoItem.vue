  ...
  <div :class="[
    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-shrink-0 mt-1">
        ...
        />
        <div v-else class="w-5 h-5 border-2 border-gray-300 rounded bg-gray-100"></div>
      </div>
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex items-start justify-between gap-2 mb-2">
          <div class="flex-1">
            <!-- 优先级标签 -->
            <span
              v-if="todo.priority >= 3 || todo.checklistPriority"
              :class="[
            ...
            <!-- Checklist分类标签 -->
            <span
              v-if="todo.fromChecklist && todo.checklistCategory"
              class="inline-block px-2 py-0.5 rounded-full text-xs font-medium mr-2 bg-purple-100 text-purple-700 border border-purple-300"
            >
              {{ todo.checklistCategory }}
            </span>
            ...
            <!-- 未展开提示 -->
            <span v-if="todo.fromChecklist && !todo.hasDetails" class="ml-2 text-xs text-gray-500">
              (点击"展开"添加详情)
            </span>
          </div>
          ...
          <div class="flex gap-1 flex-shrink-0">
            <!-- Checklist未展开时显示展开按钮 -->
            <button
              v-if="todo.fromChecklist && !todo.hasDetails"
              @click="$emit('expand', todo)"
              class="px-2 py-1 text-green-600 hover:bg-green-50 rounded text-xs font-medium transition-colors"
              title="展开添加详情"
            >
              📝 展开
            </button>
        ...
        <!-- 描述 -->
        <div v-if="todo.description && todo.hasDetails !== false" class="mt-2 text-sm text-gray-700 prose prose-sm max-w-none">
          <div v-html="renderMarkdown(todo.description)" class="markdown-content"></div>
        </div>

        <!-- 关联资源 -->
        <div v-if="todo.resourceLinks && todo.resourceLinks.length > 0 && todo.hasDetails !== false" class="mt-3 space-y-1.5">
          <div
...
defineEmits(['toggle-complete', 'edit', 'delete', 'expand'])
